...
def surface_code_circuit_string(distance: int, 
    ...
    Args:
        distance (integer): Code distance for the generated circuit
        rounds (integer): Number of complete rounds in the generated circuit
        p (float): Uniform physical error rate for all qubits
        p_def (array): Array of physical error rates for defective qubits.
        def_coord (array): Array of coordinate pairs specifying defective qubits (i.e. `[(x1, y1), (x2, y2), ...]`). The defect at `def_coord[i]` will have an error rate of `p_def[i]`.
    """
...
def surface_code_circuit_string_with_sigma_noise(distance: int, 
    ...
    Args:
        distance (integer): Code distance for the generated circuit
        rounds (integer): Number of complete rounds in the generated circuit
        p_mu (float): Mean physical error rate for all qubits
        p_sigma (float): Deviation of physical error rates from `p_mu` for all qubits
        p_def (array): Array of physical error rates for defective qubits.
        def_coord (array): Array of coordinate pairs specifying defective qubits (i.e. `[(x1, y1), (x2, y2), ...]`). The defect at `def_coord[i]` will have an error rate of `p_def[i]`.
        noise_distrubtion_func (lambda): Function for specifying how the distrubtion of noise is generated
        show_norm_plot (bool): If set to true, will plot the circuits noise distrubtion
        return_qubit_mapping (bool): If set to true, function will return the index-to-error and coordinate-to-index mapping of the underlying circuit
    """
    ...
    i2e = prepare_error_map(distance=distance, noise_dist_func=noise_distrubtion_func, p_mu = p_mu, p_sigma=p_sigma)
